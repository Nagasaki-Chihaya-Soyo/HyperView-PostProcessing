    ...
    if { [catch {
        switch $cmd {
            ...
            "add_slide_one_image_only" {
                ...
                if { [catch {
                    set native_file_path [file nativename $file_path]
                    hwc report Report add slide "One Image only" "label=$label"
                    hwc report Report edit items image "position=$position" source=file "file=$native_file_path"
                    hwc report Report edit items slide "position=$native_file_path" "label=Analyst [lindex $label end]"
                } err] } {
